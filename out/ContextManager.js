...
class ContextManager {
     ...
     */
    async getCompressedContext(userQuery) {
        const context = {
...
class LCCPService {
    constructor(config) {
        ...
        this.reverseManifest = new Map();
    }
    transpile(context, userQuery) {
    ...
    compressWord(word) {
        if (word.length < 4 || word.includes('_') || word.startsWith('$') || word.includes(':'))
            return word;
        const first = word.charAt(0);
        const rest = word.slice(1).replace(/[aeiouAEIOU]/g, '');
        return first + rest;
    }
    processValue(data, depth) {
        if (data === null || data === undefined)
            return '';
        if (depth > 2 && typeof data === 'object')
            return this.toMinifiedYaml(data, depth);
        if (Array.isArray(data))
            return this.arrayToTOON(data, depth);
        if (typeof data === 'object')
            return this.objectToHybrid(data, depth);
        return typeof data === 'string' ? this.applyTSC(data) : String(data);
    }
    arrayToTOON(arr, depth) {
        if (arr.length === 0)
            return '[]';
        // Check for objects
        ...
        if (isObjArr) {
            ...
            if (hasContent)
                return this.toMinifiedYaml(arr, depth);
    ...
    toMinifiedYaml(data, level = 0) {
        const indent = '  '.repeat(level);
        if (Array.isArray(data)) {
            // Flow style for simple lists, block for complex
            if (data.every(i => typeof i !== 'object') && data.length < 5)
                return `[${data.join(',')}]`;
